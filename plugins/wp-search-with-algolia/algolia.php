...
function getparentpage()
{
	...
	$posts = $wpdb->get_results("Select * from $wpdb->posts where post_title in (" . join(',',$titlearray) . ") and post_status = 'publish'");
	...
	foreach($posts as $post)
	{
		if($post->post_parent)
		{
			...
			if($parent)
			{
				$parenttitle[$post->post_title] = $parent->post_title;	
			}		
		}

	}

	echo json_encode($parenttitle);
	wp_die();
}
...
function algolia_load_textdomain() {
	...
	add_action('wp_ajax_getparent','getparentpage');
	add_action('wp_ajax_nopriv_getparent','getparentpage');
}
